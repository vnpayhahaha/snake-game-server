...
class ControllerGenerator extends MineGenerator implements CodeGenerator
{
    ...
    public function generator(): void
    {
        ...
        if ($this->tablesContract->getGenerateType() === GenerateTypeEnum::ZIP) {
            $path = BASE_PATH . "/runtime/generate/php/app/Http/Admin/Controller/{$module}/";
        } else {
            $path = BASE_PATH . "/app/Http/Admin/Controller/{$module}/";
        }
...
//        }
    ...
    {
        $keyAndContent = array_combine($this->getPlaceHolderContent(), $this->getReplaceContent());
        file_put_contents("./keyandcontent.json", json_encode($keyAndContent));
        $pattern = '/\{[^}]+\}/';
        $values = $this->getReplaceContent();
        foreach ($values as &$tlp) {
            preg_match_all($pattern, $tlp, $matches);
            if (empty($matches[0])) {
                continue;
            }
            foreach ($matches[0] as $key) {
                if (isset($keyAndContent[$key])) {
                    $tlpText = $keyAndContent[$key];
                    $tlp = str_replace($key, $tlpText, $tlp);
                }
            }
        }
    ...
    {
        return "App\\Http\\Admin\\Controller\\{$this->tablesContract->getPackageName()}";
    }
    ...
    {
        $packageName = $this->tablesContract->getPackageName();
        return <<<UseNamespace
use App\\Service\\{$packageName}\\{$this->getBusinessName()}Service;
use App\\Http\\Admin\\Request\\{$packageName}\\{$this->getBusinessName()}Request;
use App\\Schema\\{$packageName}\\{$this->getBusinessName()}Schema;
UseNamespace;
    }
    ...
    private function OPERATION_ID(): string
    {
        $OPERATION_ID = "";
        foreach (explode("/", $this->getControllerRoute()) as $value) {
            if ($value) {
                $OPERATION_ID .= Str::upper(Str::substr($value, 0, 1)) . Str::lower(Str::substr($value, 1));
            }
        }
        return $OPERATION_ID;
    }
