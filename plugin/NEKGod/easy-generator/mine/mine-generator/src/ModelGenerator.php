...
class ModelGenerator extends MineGenerator implements CodeGenerator
{
    ...
    public function generator(): void
    {
        ...
        if ($this->tablesContract->getGenerateType()->value === 1) {
            $path = BASE_PATH . "/runtime/generate/php/app/Model/{$module}";
        } else {
            $path = BASE_PATH . "/app/Model/{$module}";
        }
        ...
        if ($application->run($input, $output) === 0) {
            ...
            if ($modelName[strlen($modelName) - 1] == 's' && $modelName[strlen($modelName) - 2] != 's') {
                ...
                $oldPath = BASE_PATH . "/app/Model/{$module}/{$oldName}.php";
                $sourcePath = BASE_PATH . "/app/Model/{$module}/{$modelName}.php";
                $this->filesystem->put(
                    $sourcePath,
            ...
            // 压缩包下载
            if ($this->tablesContract->getGenerateType()->value === 1) {
                $toPath = BASE_PATH . "/runtime/generate/php/app/Model/{$module}/{$modelName}.php";
                try{
                    $this->filesystem->makeDirectory(BASE_PATH . "/runtime/generate/php/app/Model/{$module}", 0755, true);
                }catch (\Throwable $e){}
