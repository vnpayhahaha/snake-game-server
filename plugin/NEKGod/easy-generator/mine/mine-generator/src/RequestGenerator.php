...
class RequestGenerator extends MineGenerator implements CodeGenerator
{
    ...
    public function generator(): void
    {
        ...
        if ($this->tablesContract->getGenerateType()->value === 1) {
            $path = BASE_PATH . "/runtime/generate/php/app/Http/Admin/Request/{$module}/";
        } else {
            $path = BASE_PATH . "/app/Http/Admin/Request/{$module}/";
        }
    ...
    {
        return "App\\Http\\Admin\\Request\\{$this->tablesContract->getPackageName()}";
    }
...
//        );
    ...
    {
        $phpCode = '';
        $path = $this->getStubDir() . '/Request/messages.stub';
        foreach ($this->columns as $column) {
            $phpCode .= $this->getMessagesCode($column);
        }
        return str_replace('{LIST}', $phpCode, $this->filesystem->sharedGet($path));
    }

    protected function getMessagesCode(array &$column): string
    {
        $space = '            ';
        return sprintf(
            "%s'%s' => '%s',\n",
