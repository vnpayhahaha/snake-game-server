...
use Mine\Generator\SchemaGenerator;
use Mine\Generator\VueDataCommonGenerator;
use Mine\Generator\VueDataFormItemsGenerator;
use Mine\Generator\VueDataSearchItemsGenerator;
use Mine\Generator\VueDataTableGenerator;
use Mine\Generator\VueFormGenerator;
use Mine\Helper\Str;
use Plugin\NEK\CodeGenerator\Mapper\SettingGenerateTablesMapper;
...
//        }
    ...
    public function updateTableAndColumns(array $data): bool
    {
        if (empty($data['package_name'])) {
            throw new BusinessException(ResultCode::UNPROCESSABLE_ENTITY, "包名不许为空");
        }
        $id = $data['id'];
        ...
        $names = explode('/', str_replace("\\", "/", $data['package_name']));
        foreach ($names as &$name){
            $name = Str::title(mb_substr($name, 0, 1)) . mb_substr($name, 1);
        }
        $data['package_name'] =  implode("\\", $names);
        $data['namespace'] = "{$data['package_name']}";
        $data['module_name'] = str_replace("\\", "/",$data['package_name']);
    ...
    protected function generateCodeFile(mixed $id, int $adminId): SettingGenerateTables
    {
        ...
        $classList = [
            ...
            VueFormGenerator::class,
            VueDataFormItemsGenerator::class,
            VueDataSearchItemsGenerator::class,
            VueDataTableGenerator::class,
            VueDataCommonGenerator::class,
