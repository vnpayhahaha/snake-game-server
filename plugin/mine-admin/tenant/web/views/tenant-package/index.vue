...
import { deleteByIds, page } from '$/mine-admin/tenant/api/tenantPackage'
...
defineOptions({ name: 'plugin:mine-admin:tenant-package' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => '套餐管理',
    subTitle: () => '为租户分配套餐，套餐可设置租户所拥有的菜单权限',
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['plugin:mine-admin:tenant:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['plugin:mine-admin:tenant:delete']"
          type="danger"
          plain
