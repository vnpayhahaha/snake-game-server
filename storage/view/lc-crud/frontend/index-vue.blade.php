...
import { deleteByIds, page } from '~/{{ $packageName }}/api/{{ $camelCaseName }}.ts'
...
defineOptions({ name: '{{$packageName}}:{{Str::snake($camelCaseName)}}' })
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['{{$packageName}}:{{Str::snake($camelCaseName)}}:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['{{$packageName}}:{{Str::snake($componentName)}}:delete']"
          type="danger"
          plain
      ...
      <template #empty>
        <el-empty>
          <el-button
            v-auth="['{{$packageName}}:{{Str::snake($componentName)}}:save']"
            type="primary"
            @click="() => {
              maDialog.setTitle(t('crud.add'))
              maDialog.open({ formType: 'add' })
